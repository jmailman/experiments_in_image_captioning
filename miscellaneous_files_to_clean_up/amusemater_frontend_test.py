...
#     #return "Test string"
#
# def construct_caption( row, phrase_dict_ ):
#     return sub( phrase_dict_[ row['phrase_id']], row['phonetic_match'],  row['semantic_match']  )
...
def construct_caption_by_substitution(row_, phon_match, sem_match, phrase_dict_ ):
    ...
    #return phrase_dict_[ row_['phrase_id'] ].text_string
...
def get_best_captions(df, phrase_dict_, selection_size=25):
    ...
    #best_df['caption'] = best_df.apply(lambda row: construct_caption(row, phrase_dict_), axis=1 )
    #best_df = best_df.assign(caption = lambda row: construct_caption(row, phrase_dict_), axis=1 )
    #  best_df = best_df.assign(caption = lambda row: sub( phrase_dict_[ row['phrase_id']], row['phonetic_match'],  row['semantic_match']  ), axis=1 )

    #best_df = best_df.assign(caption = "Test string yada yada", axis=1 )          # THESE THREE LINES WORK BUT THOSE ABOVE DO NOT.
    ...
    #best_df.loc[:, 'caption'] = "Test string mary had a little lamb"              # I never noticed this before.
...
#def process_captioning_the_image( phrase_dict_ ):
...
show_other_captions = st.checkbox('Show other captions', value=False)
