...
def make_phon_fams_and_sem_family( raw_w, nearby_words_dict_, label_word_cat_dict_  ):
    w = raw_w.lower()
    w_words = w.split('_')
    ...
    if subtle and (word_record_.word in nearby_words_dict_.keys() ):
        ...
        # use w2vec words now
        try:
            if label_word_cat_dict_[raw_w] == 'object':
                for w_ in w2v_model_bow.most_similar(positive = [*w_words, 'walks'], negative=['man'], topn=4):
                    if (w_[0] not in raw_w) and  (word_record_.word not in w_ ):
                        near_word = Close_word(w_[0], 0.15)
                        phon_fams_list.append( make_phon_fam_for_sem_fam_member( near_word ) )

                for w_ in w2v_model_skp.most_similar(positive = [*w_words, 'walks'], negative=['man'], topn=4): 
                    if (w_[0] not in raw_w) and  (word_record_.word not in w_[0] ):
                        near_word = Close_word(w_[0], 0.15)
                        phon_fams_list.append( make_phon_fam_for_sem_fam_member( near_word ) )


            elif label_word_cat_dict_[raw_w] == 'animal':
                for w_ in w2v_model_bow.most_similar(positive = [*w_words, 'walks'], negative=['man'], topn=4):
                    if (w_[0] not in raw_w) and  (word_record_.word not in w_[0] ):
...
def generate_the_captions( bar_ ):
    ...
    with open("data/" + 'label_word_cat_dict.pickle', 'rb') as to_read:  
        label_word_cat_dict_  =  pickle.load(to_read)      
...
#   image_topic_word_ = image_topics_
    image_sem_fam = make_phon_fams_and_sem_family( image_topic_word_, nearby_words_dict_, label_word_cat_dict_ )
    ...
    return cand_df_, image_topic_word_, phrase_dict_, label_word_cat_dict_
    #return

# import streamlit as st
# bar = st.progress(0)    
# #generate_the_captions(bar )
# cand_df_, image_topic_word_, phrase_dict_, label_word_cat_dict_ = generate_the_captions(bar )
...
cand_df, image_topic_word, phrase_dict, label_word_cat_dict = generate_the_captions(bar )
