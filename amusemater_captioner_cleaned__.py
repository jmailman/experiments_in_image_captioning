...
def make_phon_fam_for_sem_fam_member( w_record, fuzzy_meta_dict, fuzzy_nysiis_dict, syllable_count_dict, thresh=3 ):
    ...
    for word in words_set:
        ...
        if (phon_dist <= thresh) and (word != w_record.word):
            ...
            first_let_disc = last_letter_discount(word, w_record.word)
            close_word_list.append( Close_word(word.lower(), phon_dist + non_rhyme_penalty + syll_pen - last_let_disc- first_let_disc ))
    ...
    for word in rhyme_word_list:
            ...
            first_let_disc = last_letter_discount(word, w_record.word)
            last_let_disc = last_letter_discount(word, w_record.word)
            close_word_list.append( Close_word(word, rhyme_dist + syll_pen - last_let_disc-first_let_disc) )
...
def make_phon_fams_and_sem_family( w ):
    ...
    for close_w_record in sem_sim_words:
        ...
        phon_fams_list.append( make_phon_fam_for_sem_fam_member( close_w_record, fuzzy_meta_dict, fuzzy_nysiis_dict, syllable_count_dict   ) )
